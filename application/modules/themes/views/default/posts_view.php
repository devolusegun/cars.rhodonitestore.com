    ...
    <div class="col-md-9">
        ...
        <div class="agent-container" id="panel">
            ...
            foreach($posts->result() as $post){ 
                ...
                $description = get_blog_data_by_lang($post,'description');

            ?>
            <div class="agent-holder clearfix">
                <h4><a href="<?php echo site_url('show/postdetail/'.$post->id.'/'.dbc_url_title($title));?>"><?php echo $title;?></a></h4>
                ...
                <a href="<?php echo site_url('show/postdetail/'.$post->id.'/'.dbc_url_title($title));?>"><img alt="<?php echo $title;?>" src="<?php echo get_featured_photo_by_id($post->featured_img);?>" class="post-thumb"></a>
                <?php echo truncate(strip_tags($description),400,'&nbsp;<a href="'.site_url('show/postdetail/'.$post->id.'/'.dbc_url_title($title)).'">'.lang_key('view_more').'</a>',false);?>

                <?php $detail_link = site_url('show/postdetail/'.$post->id.'/'.dbc_url_title($title)); ?>
