		...
		<div style="clear:both"></div>
            ...
            <div class="tabbable">
                <ul class="nav nav-tabs" id="myTab1">
                    <?php $flag=1; foreach ($langs->result() as $lang){ 
                    	?>
                ...
                <div class="tab-content" id="myTabContent1">
                     ...
                     <div id="<?php echo $lang->short_name;?>" class="tab-pane fade in <?php echo (default_lang()==$lang->short_name)?'active':'';?>">
						...
						<?php 
							...
							if(set_value('title_'.$lang->short_name)!='')

								$title = set_value('title_'.$lang->short_name);

							else if(isset($page) && isset($page->title))
							{
								$data = json_decode($page->title);
								$title = $data->{$lang->short_name};
							}
						...
						<div class="form-group">
							...
							<label class="col-sm12 col-lg-12 control-label"><?php echo lang_key('Post Title');?></label>

							<div class="col-sm-12 col-lg-12 controls">

								<input type="text" class="form-control" name="title_<?php echo $lang->short_name;?>" id="title_<?php echo $lang->short_name;?>" value="<?php echo $title;?>" placeholder="<?php echo lang_key('Type something');?>" />

								<span class="help-inline">&nbsp;</span>

								<?php echo form_error('title_'.$lang->short_name); ?>
						...
						<div class="form-group">
							...
							<div class="col-sm-12 col-lg-12 controls">
								...
								<?php 
									...
									if(set_value('description_'.$lang->short_name)!='')

										$description = set_value('description_'.$lang->short_name);

									else if(isset($page) && isset($page->description))
									{
										$data = json_decode($page->description);
										$description = $data->{$lang->short_name};
									}

								?>		

								<textarea name="description_<?php echo $lang->short_name;?>" class="rich" style="height:434px"><?php echo $description;?></textarea>

								<span class="help-inline">&nbsp;</span>

								<?php echo form_error('description_'.$lang->short_name); ?>
