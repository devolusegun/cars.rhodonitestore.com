...
class Account_core extends CI_Controller {
...

		{
			...
			{				
				...
				$msg = '<div class="alert alert-danger">
					        ...
					        <strong>Username or Password does not match our records</strong>
	...
	function register()
	{
		...
		else
		{
			...
			/** Here CODEINE Made Autohub not manual approval, rather Automation account confirmed for all new users */
	...
	public function terms_check($str)
	{
		...
		{
			$this->form_validation->set_message('terms_check', 'You must accept Terms our and Condition.');
			return FALSE;
		}
		else
		{
	...
	public function send_confirmation_email($data=array('username'=>'admin','useremail'=>'awaretube@gmail.com','confirmation_key'=>'1234'))#removed "shimulcsedu@gmail.com"
	{
		$val = $this->get_admin_email_and_name();
		$admin_email = $val['admin_email'];
		$admin_name  = $val['admin_name'];
	...
	public function confirm($email='',$code='')
	{
		...
		{
			$this->session->set_flashdata('msg', '<div class="alert alert-danger">Confirmation failed. Confirmation email and code doesn\'t match.</div>');
			redirect(site_url('account/showmsg'));
		}
	}
